...
for e, L in enumerate(Ls_N):
    ...
    CR3BP_N = ThreeBodySystem(G=G, body1=cuerpo1, body2=cuerpo2, body3=satelite, potential="Newton")
    CR3BP_N.plot_CR3BP(tf, title=f"Newton L_{e+1}, $\mu=${mu}_x", save=True, plot=False)
...
for e, L in enumerate(Ls_N):
    ...
    satelite = Body("cubesat", mass=10, pos=pos_satelite)
    CR3BP_N = ThreeBodySystem(G=G, body1=cuerpo1, body2=cuerpo2, body3=satelite, potential="Newton")
    CR3BP_N.plot_CR3BP(tf, title=f"Newton L_{e+1}, $\mu=${mu}_y", save=True, plot=False)
