...
def f(x, y, mu):
    sol = (x**2+y**2)/2 - (1-mu)*np.log(np.sqrt((x+mu)**2+y**2)) - mu*np.log(np.sqrt((x-1+mu)**2+y**2))
    return sol


def fx(x, y, mu):
    sol = x-(1-mu)*(x+mu)/((x+mu)**2+y**2) - mu*(x-1+mu)/((x-1+mu)**2+y**2)
    return sol


def fy(x, y, mu):
    sol = y-(1-mu)*y/((x+mu)**2+y**2)-mu*y/((x-1+mu)**2+y**2)
    return sol
